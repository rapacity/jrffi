...
(begin-for-syntax
  ...
  (define (java-module-path stx)
    ...
    (define (current-source-location)
      (define loc (source-location-source stx))
      (if (symbol? loc)
          (current-directory)
          (directory-path loc)))
    (define package (syntax->datum stx))
    (define name (symbol->string package))
    (define file (relative-binding-file (symbol->string package) (current-source-location)))
